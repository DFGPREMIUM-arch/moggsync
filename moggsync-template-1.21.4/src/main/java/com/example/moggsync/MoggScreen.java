...
public class MoggScreen extends Screen {
    ...
    private void bMain(int y){
        ...
        field(y,"Телепорт после смерти",cfg.deathTeleportCmd,40,false,s->cfg.deathTeleportCmd=s.strip());y+=ROW;
        toggle(y,"Авто-переподключение",()->cfg.autoReconnect,v->cfg.autoReconnect=v);y+=ROW;
        note("Смерть: авто-нажимает «Возродиться», потом телепортирует командой.",y+5);
        note("Пароль хранится локально в config/moggsync.json.",y+16);}
