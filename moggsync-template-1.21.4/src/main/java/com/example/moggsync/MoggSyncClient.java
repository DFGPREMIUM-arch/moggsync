...
public class MoggSyncClient implements ClientModInitializer {
    ...
    private void tickRitualReport(long now) {
        ...
        totalRilliki += gained;
        String t2 = LocalTime.now().format(DateTimeFormatter.ofPattern("HH:mm:ss"));
        String srv2 = mc().getCurrentServerEntry()!=null ? mc().getCurrentServerEntry().address : "сервер";
        pendingCaption = "<b>🎉 РИТУАЛ #" + ritualsDone + " ЗАВЕРШЁН!</b>\n"
                + "━━━━━━━━━━━━━━━\n"
                + "👤 <code>" + escHtml(me) + "</code>\n"
                + "🤝 <code>" + escHtml(cfg.partnerName) + "</code>\n"
                + "━━━━━━━━━━━━━━━\n"
                + (gained > 0 ? "💰 <b>+" + gained + " рилликов</b>\n" : "")
                + "📊 Всего: <b>" + totalRilliki + " риллик</b>\n"
                + "🏆 Сессия: <b>" + ritualsDone + " ритуалов</b>\n"
                + "📍 <i>" + escHtml(srv2) + "</i>\n"
                + "⏰ <i>" + t2 + "</i>\n"
                + (rewardLines.isEmpty() ? "" : "<code>" + escHtml(String.join("\n", rewardLines)) + "</code>");
        screenshotAt = now;
    ...
    private void tgSayHtml(String html) { TelegramReporter.sendHtml(cfg, html); }

    private String statusText() {
        var srv = mc().getCurrentServerEntry();
        return "<b>📊 Статус MoggSync</b>\n"
                + "━━━━━━━━━━━━━━━\n"
                + "👤 <code>" + escHtml(account) + "</code>\n"
                + "🤝 <code>" + escHtml(cfg.partnerName.isBlank()?"не задан":cfg.partnerName) + "</code>\n"
                + "━━━━━━━━━━━━━━━\n"
                + (enabled ? "🟢 <b>Мод включён</b>" : "🔴 <b>Мод выключен</b>") + "\n"
                + "⏱ " + timerText() + "\n"
                + "📍 " + escHtml(srv==null?"не на сервере":srv.address) + "\n"
                + "━━━━━━━━━━━━━━━\n"
                + "🏆 Ритуалов: <b>" + ritualsDone + "</b>\n"
                + "💰 Рилликов: <b>" + totalRilliki + "</b>";
    }
    ...
    private void telegramCommand(String cmd, String arg, boolean inGame, long now) {
        switch (cmd) {
            case "start", "help" -> tgSayHtml(String.join("\n",
                    "<b>Управление MoggSync</b>",
                    "",
                    "Любой текст → в игровой чат",
                    "<code>!warp farm</code> → выполнить команду",
                    "",
                    "<b>Команды:</b>",
                    ...
                    "/lobby — войти через лобби",
                    "/chat all | me | off — чат в Telegram",
                    "/cmd текст — команда в игре",
                    "/say текст — сообщение в чат"));
            case "status" -> tgSayHtml(statusText());
